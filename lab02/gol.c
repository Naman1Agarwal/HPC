...
value_t nodeUpdate(value_t** m, index_t i, index_t j){
    ...
    for (index_t testi = i-1; testi++; testi <= i+1){
        for (index_t testj = j-1; testj++; testj <= j+1){
            if (m[testi][testj] == 1){
                alive++;
            }
        }
...
*/ 
void update(matrix* m1, matrix* m2){
...
void iterate(simulation* sim){
    ...
    for (int i = 0; i < sim->generations; i++){
